...
- 通常・スムーズ・グラデーション・投げ縄の4種類の独自ウェイトブラシ
- 正規化・小数点整理・影響数制限・閾値整理・違反整理
- スムーズ化・ミラー実行・レスト位置変更
- コピー&ペースト・近接転送・頂点グループ間転送
- 骨取得・骨トランスフォーム・ボーンハイライト・表示補助の切り替え
- Blender標準方式と、独自ボクセル拡散方式による2種類の自動ウェイト
...
- `個` をクリックすると、対象の行の値がロックされます。
- `頂点`クリックすると、その頂点行がビュー上でハイライト表示され、グリッド上で常に表示状態になります。
...
併用して、骨取得や骨トランスフォームも実行できるため、ウェイト作業が迅速にできます。
...
骨トランスフォームの右クリックドラッグ以外、すべてのショートカットは変更可能です。
