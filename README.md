...
対象範囲は選択部位です。複数の開いた辺列が同じメッシュを横切って"切れ目"になっている場合は、その間に挟まれた頂点も、帯を囲む全経路の生成骨でまとめてウェイト付けされます（選択辺を境界に 2 領域以上へ接する場合のみ。短い無関係な選択でメッシュ全体が対象になるのを防ぎます）。非連結の別メッシュは対象外で、`中心軸` も同じ範囲を使います。
...
- `計算方式` で稼働方式を選択 : `高速`（カーソル下の接続辺の周辺だけを高速処理）/ `表面`（接続トポロジーをたどり、裏面へ貫通しない）/ `ボリューム`（空間的な近接頂点も参照し、局所密度に合わせる。`ボリューム範囲` で届く範囲を調整）。
...
**影響数整理設定**（影響数を上限に収めるときの残し方）

- `親子関係を考慮`（デフォルト）: 骨の枝を考慮し、上限に余裕があれば各枝から少なくとも 1 つを残します。削除したウェイトは可能な限り同じ枝へ戻します。
- `値を優先` : ウェイト値の大きい影響から残します（従来方式）。
- `近似ウェイト幅` : 同じ枝の中で順番を入れ替えてよいウェイト差を設定。階層に対応しないグループは自動的に値優先へフォールバック。
