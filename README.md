...
<p align="center">
  <img width="1346" height="790" alt="Image" src="https://github.com/user-attachments/assets/a5dfad75-f64e-44ce-a632-4160785ebcdd" />
</p>
...
- 個列のセルをクリックすると、対象の行の値がロックされます。
- 頂点列をクリックすると、その頂点行がビュー上でハイライト表示され、グリッド上で常に表示状態になります。
- `Shift` / `Ctrl` を押しながらドラッグやクリックで、行選択を追加・解除できます。
- 個/頂点を`alt+クリック`すると対象の状態が解除されます。
...
- セルをクリックすると、そのセルの数値を直接入力できます。`Enter` で確定、キャンセルしたい場合適当な列を選択してください。
