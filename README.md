...
- `Ctrl + Shift + 左ドラッグ` : 周囲の弱い値になじませ、選択列の影響を縮小。
- `アクティブグループのみ` : 選択列だけをスムーズにし、周囲に同じ列のウェイトがあれば、0 の頂点にも広げます。
- `強さ` で寄せ具合、`回数` で反復回数を調整。
- 無視列を選択している場合は、その無視列だけを処理。

v1.5.251 では、縮小操作が `Alt + 左ドラッグ` から `Ctrl + Shift + 左ドラッグ` に変更されています。通常ブラシの `Ctrl + Shift + 左ドラッグ` は、引き続き選択列の拡張です。ウェイトペイントモード中の `Alt + 左クリック` は Blender 側の骨・頂点グループ選択へ渡されます（Blender のキー設定に従います）。
