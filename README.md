...
外部ウィンドウや追加の UI ライブラリは使いません。すべて Blender 内蔵の GPU 描画で動作します。

> **バージョン 1.5.26 時点の仕様です。**
...
- [専用ウェイトエディタウィンドウ](#専用ウェイトエディタウィンドウ)
...
- ウェイトカラープレビュー
- 3D ビューとは別の **専用ウェイトエディタウィンドウ**
- 複数メッシュの同時編集と、頂点グループ選択の同期
- 日本語 / 英語 UI（Blender の言語設定に自動追従、または個別に固定）

### ミラー・整理での中央 L/R 均等化

選択頂点ミラー・全体ミラーの実行時に、中心軸上の頂点の L/R ウェイトを自動で均等化します。
`正規化` `小数整理` `閾値整理` `影響整理` `違反整理` でも中心軸上の L/R の対称性は維持され、小数の丸めと影響数の絞り込みは、編集可能な L/R ペアを 1 つの対称単位として扱います。
...
NumPy は Blender に含まれていれば利用し、無い場合はメインスレッドのスカラー処理へフォールバックします。
...
- 行数・列数は、スクロールバーまたはホイール操作で調整できます。
...
| `×1` | クリックで `×1` → `×1.5` → `×2` を順に切り替えます。`Shift + クリック` で `0.50〜4.00` のカスタム倍率を入力できます。 |
| `×` | GPU オーバーレイを閉じます。 |

3D ビュー用と専用ウィンドウ用の HUD 倍率は別々に保存され、次に開いたときも前回の倍率が使われます。
...
- `非表示ワード` : 列名の表示から指定した文字だけを隠します。実際の頂点グループ名は変更されません。
...
- これらの整理は、中心軸上の頂点の L/R 対称性を保ったまま処理します。
...
- `Shift + クリック` : 境界スムーズ。
- `Ctrl + クリック` : 未選択の周囲に合わせて、異常なウェイトを修正します。
- 右隣の `…` から詳細設定を開けます。稼働方式、スムーズの強さ、回数、実行後の整理方法を調整できます。
...
複数のメッシュを同時に編集モードに入れている場合、グリッドのフッターに `複数編集: Nobj` と表示され、複数オブジェクトの頂点をまとめて扱えます。
...
ツールヘッダーのウィンドウアイコンから、GPU オーバーレイと同じエディタを別の Blender ウィンドウで開けます。

- 軽量なテクスチャノードエディタを描画面として使用します。
- ウィンドウのサイズと配置は Blender と OS が管理します。GPU UI の大きさは HUD 倍率ボタンで個別に調整してください。
- ビューポート表示の切り替え（`モディ` `レスト` `最前面` `オーバーレイ` など）は、このウィンドウを開いた 3D ビューに対して働きます。
- ヘッダーの `×` でウィンドウを閉じます。
...
| `GPUオーバーレイ UI倍率` | `3DビューHUD` と `専用ウィンドウHUD` の倍率を個別に設定します。 |
...
| `追加ショートカット（デフォルトOFF）` | 任意ショートカットの有効化。 |
